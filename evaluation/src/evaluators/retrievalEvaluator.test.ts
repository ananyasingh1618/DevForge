...
describe("evaluateRetrieval", () => {
  it("computes recall/hit-rate as 1.0 when the expected chunk is in top-K", async () => {
    const cases: RetrievalCase[] = [
      { id: "c1", query: "password comparison", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" },
    ];
    const report = await evaluateRetrieval(cases, 5, CHUNKS);
    expect(report.aggregate.recallAtK).toBe(1);
    expect(report.aggregate.hitRate).toBe(1);
    expect(report.cases[0]!.passed).toBe(true);
  });

  it("computes recall as 0 and reports a failure reason when the expected chunk is never returned", async () => {
    const cases: RetrievalCase[] = [
  ...
  it("computes mean reciprocal rank based on the expected chunk's actual rank position", async () => {
    ...
    const ranked = await rankChunks("password comparison hash equality", CHUNKS, 2);
  ...
  it("precision@K counts acceptable alternatives as relevant, not as false positives", async () => {
    const cases: RetrievalCase[] = [
      { id: "c1", query: "password comparison", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: ["b"], notes: "" },
    ];
    const report = await evaluateRetrieval(cases, 2, CHUNKS);
  ...
  it("reports emptyResultRate 0 when K > 0 and chunks exist", async () => {
    const report = await evaluateRetrieval([{ id: "c1", query: "anything", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" }], 3, CHUNKS);
    expect(report.aggregate.emptyResultRate).toBe(0);
  });

  it("reports emptyResultRate 1 when K is 0", async () => {
    const report = await evaluateRetrieval([{ id: "c1", query: "anything", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" }], 0, CHUNKS);
    expect(report.aggregate.emptyResultRate).toBe(1);
    expect(report.aggregate.recallAtK).toBe(0);
  });

  it("flags duplicate chunk ids within one ranking, if any were ever produced", async () => {
    ...
    const report = await evaluateRetrieval([{ id: "c1", query: "password", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" }], 5, CHUNKS);
    expect(report.aggregate.duplicateSourceCaseRate).toBe(0);
  });

  it("context-size compliance holds for small fixture chunks within the production budget", async () => {
    const report = await evaluateRetrieval([{ id: "c1", query: "password", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" }], 5, CHUNKS);
    expect(report.aggregate.contextSizeCompliant).toBe(1);
    expect(MAX_CONTEXT_CHARS).toBe(16_000);
  });

  it("is fully deterministic across repeated runs on the same input", async () => {
    const cases: RetrievalCase[] = [
      { id: "c1", query: "password comparison", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" },
    ];
    const first = await evaluateRetrieval(cases, 5, CHUNKS);
    const second = await evaluateRetrieval(cases, 5, CHUNKS);
...
describe("rankChunks — adaptive relative-score cutoff", () => {
  it("mirrors api/src/services/retrieval.ts's RELATIVE_SCORE_CUTOFF constant exactly", () => {
    ...
    expect(RELATIVE_SCORE_CUTOFF).toBe(0.82);
  });

  it("can return fewer than K chunks when the score falls off a cliff after the top result", async () => {
    const ranked = await rankChunks("where are numbers rounded to a fixed number of decimal places?", undefined, 5);
  ...
  it("never returns zero chunks when at least one chunk exists, however weak its score", async () => {
    const ranked = await rankChunks("completely unrelated gibberish query zzz qqq", CHUNKS, 5);
    expect(ranked.length).toBeGreaterThanOrEqual(1);
  });

  it("never returns more than the requested K even when many chunks score similarly", async () => {
    const ranked = await rankChunks("function", undefined, 3);
...
describe("rankChunks — coherence-aware cutoff (retrieval-target-closure architecture work)", () => {
  it(
    ...
    async () => {
      ...
      const ranked = await rankChunks("Which function does orderProcessor import to load a user's existing orders?", undefined, 5);
  ...
  it(
    ...
    async () => {
      const ranked = await rankChunks("How does DevForge check a user's password during login?", undefined, 5);
...
describe("evaluateRetrieval — rank distribution", () => {
  it("puts a case that ranks the expected chunk 1st into the top-1 bucket", async () => {
    const cases: RetrievalCase[] = [
      { id: "c1", query: "password comparison", expectedChunkIds: ["a"], acceptableAlternativeChunkIds: [], notes: "" },
    ];
    const report = await evaluateRetrieval(cases, 5, CHUNKS);
  ...
  it("puts a total miss into the 4-plus-or-missed bucket", async () => {
    const cases: RetrievalCase[] = [
      { id: "c1", query: "xyz", expectedChunkIds: ["does-not-exist"], acceptableAlternativeChunkIds: [], notes: "" },
    ];
    const report = await evaluateRetrieval(cases, 5, CHUNKS);
  ...
  it("the three rank-distribution buckets always sum to 1", async () => {
    const report = await evaluateRetrieval();
    const sum =
