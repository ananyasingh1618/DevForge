...
describe("rankWithDiagnostics", () => {
  it("ranks every fixture chunk, not just a top-K slice", async () => {
    const breakdown = await rankWithDiagnostics("password comparison");
  ...
  it("never exposes chunk content — only path/symbol/scores", async () => {
    const breakdown = await rankWithDiagnostics("password");
  ...
  it("is fully deterministic across repeated calls", async () => {
    const first = await rankWithDiagnostics("password comparison");
    const second = await rankWithDiagnostics("password comparison");
    expect(first).toEqual(second);
  });
});

describe("buildCaseDiagnostics", () => {
  it("reports the correct rank for a case whose expected chunk is returned", async () => {
    const testCase = RETRIEVAL_CASES.find((c) => c.id === "retrieval-password-check")!;
    const diag = await buildCaseDiagnostics(testCase, ["auth-verify-password", "auth-require-auth"]);
    expect(diag.firstRelevantRank).not.toBeNull();
    expect(diag.missingFromReturned).toEqual([]);
  });

  it("reports missing expected chunks when they weren't in the returned set", async () => {
    const testCase = RETRIEVAL_CASES.find((c) => c.id === "retrieval-password-check")!;
    const diag = await buildCaseDiagnostics(testCase, ["some-other-chunk"]);
    expect(diag.missingFromReturned).toEqual(["auth-verify-password"]);
  });

  it("reports a null firstRelevantRank only when truly no expected chunk exists in the fixture set", async () => {
    const fakeCase = { id: "fake", query: "x", expectedChunkIds: ["does-not-exist"], acceptableAlternativeChunkIds: [], notes: "" };
    const diag = await buildCaseDiagnostics(fakeCase, []);
