...
optional key, `VOYAGE_API_KEY`. Codebase Q&A and code review are not implemented; nothing here
fakes them. `GET /health` always works — analyze/generate requests fail with a clear 503 with
no `ANTHROPIC_API_KEY` set, and embedding requests fail with a clear 503 with no
`VOYAGE_API_KEY` set, instead of a fabricated result. See the root [README.md](../README.md)
