...
/**
 * Integration test: register -> create project -> request a code review,
 ...
 * GitHub, embedding-provider, or review-provider call is made — not a
 * fabricated review. Like tests/qa.test.ts, this suite needs no real
...
const email = `review-integration-test-${Date.now()}@example.com`;
const password = "correct-horse-battery";

describe("register -> create project -> run a code review (real stack)", () => {
  ...
  it("gets a real, honest NO_COMPLETED_INDEX response — no fabricated review", async () => {
    ...
    const createRes = await fetch(`${API_URL}/projects`, {
      ...
      body: JSON.stringify({ name: "Review Integration Test Project" }),
    });
    expect(createRes.status).toBe(201);
    const createBody = (await createRes.json()) as { data: { project: { id: string } } };
    const projectId = createBody.data.project.id;

    // Confirmed empty before running a review.
    const listBeforeRes = await fetch(`${API_URL}/projects/${projectId}/reviews`, {
      headers: { Cookie: sessionCookie },
    });
    const listBeforeBody = (await listBeforeRes.json()) as { data: { reviews: unknown[] } };
    expect(listBeforeBody.data.reviews).toEqual([]);

    // The real path a user would take: through the Node API, with no
    // repository ever connected and no codebase index ever built.
    const runRes = await fetch(`${API_URL}/projects/${projectId}/reviews`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Cookie: sessionCookie },
      body: JSON.stringify({ scope: "Review the authentication implementation for security issues." }),
    });
    expect(runRes.status).toBe(400);
    const runBody = (await runRes.json()) as { error: { code: string; message: string } };
    expect(runBody.error.code).toBe("NO_COMPLETED_INDEX");

    // Nothing was persisted from the blocked attempt.
    const listAfterRes = await fetch(`${API_URL}/projects/${projectId}/reviews`, {
      headers: { Cookie: sessionCookie },
    });
    const listAfterBody = (await listAfterRes.json()) as { data: { reviews: unknown[] } };
    expect(listAfterBody.data.reviews).toEqual([]);
  });
});
