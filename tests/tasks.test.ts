...
/**
 * Integration test: register -> create project -> generate tasks, run
 ...
 * analysis (and therefore every downstream generation step) cannot succeed
 * here (see requirements.test.ts, prd.test.ts, architecture.test.ts,
 * epics.test.ts) — meaning the project genuinely has no active epic
 * version. That is exactly the real path this suite exercises: calling
 * tasks generate on a project with no active epics must return the real,
 * honest NO_ACTIVE_EPICS error, not fabricated tasks and not a provider
 * error masking the actual dependency problem. If this suite is ever run
 * with a real key AND an active epic version already present, task
...
const email = `tasks-integration-test-${Date.now()}@example.com`;
const password = "correct-horse-battery";

describe("register -> create project -> generate tasks (real stack)", () => {
  ...
  it("gets a real, honest NO_ACTIVE_EPICS response — no fabricated tasks, no masking provider error", async () => {
    ...
    const createRes = await fetch(`${API_URL}/projects`, {
      ...
      body: JSON.stringify({ name: "Tasks Integration Test Project" }),
    });
    ...
    // the project having zero epic versions is sufficient here — this call
    // intentionally never touches /epics/generate.
    ...
    const generateRes = await fetch(`${API_URL}/projects/${projectId}/tasks/generate`, {
      method: "POST",
      headers: { Cookie: sessionCookie },
    });
    expect(generateRes.status).toBe(400);
    const generateBody = (await generateRes.json()) as { error: { code: string; message: string } };
    expect(generateBody.error.code).toBe("NO_ACTIVE_EPICS");

    // Nothing was persisted from the blocked attempt.
    const listTasksRes = await fetch(`${API_URL}/projects/${projectId}/tasks`, {
      headers: { Cookie: sessionCookie },
    });
    const listTasksBody = (await listTasksRes.json()) as { data: { versions: unknown[] } };
    expect(listTasksBody.data.versions).toHaveLength(0);
    ...
    // covered for real in ai-service/tests/test_tasks.py) is not silently
    ...
    const directAiRes = await fetch(`${AI_SERVICE_URL}/tasks/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
