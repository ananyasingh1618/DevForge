...
/**
 * Integration test: register -> create project -> generate PRD, run against
 * genuinely running API and ai-service processes over real HTTP (see
 ...
 * analysis itself cannot succeed here (see requirements.test.ts) — meaning
 * the project genuinely has no active requirements version. That is exactly
 * the real path this suite exercises: calling PRD generate on a project
 * with no active requirements must return the real, honest
 * NO_ACTIVE_REQUIREMENTS error, not a fabricated PRD and not a provider
 * error masking the actual dependency problem. If this suite is ever run
 * with a real key AND an active requirements version already present, PRD
...
const email = `prd-integration-test-${Date.now()}@example.com`;
const password = "correct-horse-battery";

describe("register -> create project -> generate PRD (real stack)", () => {
  ...
  it("gets a real, honest NO_ACTIVE_REQUIREMENTS response — no fabricated PRD, no masking provider error", async () => {
    ...
    const createRes = await fetch(`${API_URL}/projects`, {
      ...
      body: JSON.stringify({ name: "PRD Integration Test Project" }),
    });
    ...
    // the project having zero requirements versions is sufficient here —
    // this call intentionally never touches /requirements/analyze.
    const listRequirementsRes = await fetch(`${API_URL}/projects/${projectId}/requirements`, {
      headers: { Cookie: sessionCookie },
    });
    const listRequirementsBody = (await listRequirementsRes.json()) as {
      data: { versions: unknown[] };
    };
    expect(listRequirementsBody.data.versions).toHaveLength(0);

    // The real path a user would take: through the Node API.
    const generateRes = await fetch(`${API_URL}/projects/${projectId}/prd/generate`, {
      method: "POST",
      headers: { Cookie: sessionCookie },
    });
    expect(generateRes.status).toBe(400);
    const generateBody = (await generateRes.json()) as { error: { code: string; message: string } };
    expect(generateBody.error.code).toBe("NO_ACTIVE_REQUIREMENTS");
    ...
    // covered for real in ai-service/tests/test_prd.py) is not silently
    ...
    const directAiRes = await fetch(`${AI_SERVICE_URL}/prd/generate`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        requirements: {
