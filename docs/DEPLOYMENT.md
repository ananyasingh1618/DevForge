...
- [ ] Set `ANTHROPIC_API_KEY` / `VOYAGE_API_KEY` if AI-backed features (requirements/PRD/architecture/epics/QA/code review, embeddings) are needed in that deployment — both are honestly optional; without them those specific routes return `503 PROVIDER_NOT_CONFIGURED` and everything else still works.
