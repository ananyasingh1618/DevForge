...
- No changes to `docker-compose.yml` or `api/Dockerfile` were needed. PRD reuses the
  `AI_SERVICE_URL`/`ANTHROPIC_API_KEY` wiring Phase 2 already added, and the api container
  already runs `prisma migrate deploy` on startup, which picks up the new
  `add_prd_versions` migration automatically — confirmed below, not just assumed from reading
  the Dockerfile.
- Commands run and results (all against a genuinely rebuilt, volume-wiped stack, mirroring the
  exact procedure Phase 2's Milestone 7 used):
  ...
  - `psql \dt` → `prd_versions` present alongside `users`, `sessions`, `projects`,
    `requirements_versions` — both migrations ran automatically on the api container's
    startup, from a completely empty volume.
  ...
  - Registered a user and created a project through the **containerized** API, then called
    `POST .../prd/generate` with no requirements yet → real `400 NO_ACTIVE_REQUIREMENTS`.
    Seeded an active requirements version directly via Prisma against the Docker-mapped
    Postgres port, re-called generate → real `503 AI_PROVIDER_UNAVAILABLE` ("...to enable PRD
    generation.") — proving the PRD generation call correctly reaches the **containerized**
    ai-service over the Docker-internal hostname (`http://ai-service:8001`), not just that the
    dependency check works.
  - A throwaway Playwright script drove the **Dockerized frontend** (port 4173, a real static
    production build) through register → create project → the PRD section's blocked state
    rendered correctly with the real dependency message. One console entry was logged (a 401
    from `useAuth`'s own `meRequest()` session check on page load for an unauthenticated
    visitor) — pre-existing behavior unrelated to this phase's changes, not a new error
    introduced by PRD; noted here for accuracy rather than omitted.
  - `cd tests && API_URL=http://localhost:4000 AI_SERVICE_URL=http://localhost:8001 DATABASE_URL=postgresql://devforge:devforge@localhost:5433/devforge pnpm test`
    (against the Dockerized stack) → `3 files, 4 passed` (register-login-project, requirements,
    prd).
  ...
  - Restarted `postgres` alone for local dev; re-ran `./scripts/setup-test-db.sh` to recreate
    `devforge_test` (wiped by the earlier `down -v`) — output confirmed all three migrations,
    including `add_prd_versions`, applied to it.
  - Final full-workspace check: root `pnpm -r typecheck` → clean (api, frontend, tests). Root
    ...
    `pnpm --filter @devforge/api test` → `60 passed`; `pnpm --filter @devforge/frontend test` →
    `30 passed` (the `tests/` package's own suite requires live api/ai-service processes, which
...
- Commit: `<pending>` — "chore: verify PRD phase against a clean-volume Docker rebuild".
